...
public class DBService {	
	...
	public <E> void deleteList(List<E> objects)
	{
	...
	public <E> List<E> loadAll(Class<E> type, String condition) {
		return ofy().load().type(type).order(condition).list();
	}
	public <E> List<E> load(Class<E> type, String feild, Object value, String order)
	{
		return ofy().load().type(type).filter(feild+" ==",value).order(order).list();
	}
}
